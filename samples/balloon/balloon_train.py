...
COCO_DATAS_PATH = os.path.join(ROOT_DIR, "datasets", "balloon")

# Path to trained weights file
COCO_WEIGHTS_PATH = os.path.join(ROOT_DIR, "mask_rcnn_coco.h5")

# Path to test image file
TEST_IMG_PATH = os.path.join(COCO_DATAS_PATH, "val", "2917282960_06beee649a_b.jpg")
...
if __name__ == '__main__':
    ...
    # Validate arguments
    if command == "train":
        assert COCO_DATAS_PATH, "Argument --datasets is required for training"

    print("Weights: ", COCO_WEIGHTS_PATH)
    print("Dataset: ", COCO_DATAS_PATH)
    ...
    # number of classes
    tmp_path = os.path.join(DEFAULT_LOGS_DIR, "balloon20230112T1151", "mask_rcnn_balloon_0010.h5")
    ...
    # Train or evaluate
    if command == "train":
        train(model, COCO_DATAS_PATH)
    else:
        print("'{}' is not recognized. "
              "Use 'train' or 'splash'".format(command))

    # # test model
    # detect_and_color_splash(model, TEST_IMG_PATH)

    # Read image
    original_image = skimage.io.imread(TEST_IMG_PATH)
    # get model inference result
    results = model.detect([original_image], verbose=1)
    # Validation datasets
    dataset_val = BalloonDataset()
    dataset_val.load_balloon(COCO_DATAS_PATH, "val")
    dataset_val.prepare()
    # detect
    r = results[0]
